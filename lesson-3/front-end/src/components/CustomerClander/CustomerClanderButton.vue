<template>
    <div>
            ...
            <div class="media-content">
                ...
                <div class="bdt">
                    <div class="media-item-list">
                        ...
                        <div v-else class="media-item" :class="{ disabled: textSum === 0 }" @click="confirmText('TEXT')">
                            <img
                                src="@/assets/images/knowledgeBase/text-src.svg"
                                alt="文本"
                            >
                        ...
                        <div class="media-item" :class="{ disabled: imageSum === 0 || calculateNum.imageLength === 0 && hasMedia}"
                        ...
                        >
                            ...
                            <div class="media-item-sum">
                                {{ calculateNum.imageLength }}/{{ imageSum }}
                            </div>
                        </div>
                        <div class="media-item" :class="{ disabled: otherSum === 0 }"
        ...
        <CustomerDialog :title="`添加${title}`" :dialog-visible="dialogVisible" width="400px" :confirm-fn="mediaConfirm" @cancel="cancelDialog">
            <ImageUpload :width="350" :height="140" :type="title" :ext="ext" :size="size" :url="file.file_url" :file-type="fileType" :task-type="props.taskType" @on-success="uploadImg" @removeUrl="removeUrl" />
        </CustomerDialog>
        ...
        <!-- <CustomerDialog title="添加小程序" :dialog-visible="miniprogramVisible" width="480px" :confirm-fn="miniConfirm" @cancel="cancelDialog">
            ...
            >
                ...
                <el-form-item label="小程序封面：" prop="file_id">
                    <ImageUpload :width="436" :height="140" :type="title" :ext="ext" :size="size" :url="miniForm.file_url" @on-success="uploadImg" />
                </el-form-item>
...
const textSum = ref(1)
const otherSum = ref(1)
const imageSum = ref(9)
...
watch(() => props.list, async val => {
    ...
    let sum = 1
    textSum.value = sum - calculateNum.textLength
    if (calculateNum.imageLength > 0 || calculateNum.fileLength > 0 || calculateNum.videoLength > 0 || calculateNum.webLinkLength > 0) {
        otherSum.value = 0
        hasMedia.value = true
    } else {
        otherSum.value = sum - calculateNum.imageLength - calculateNum.fileLength - calculateNum.videoLength - calculateNum.webLinkLength
        hasMedia.value = false
    }
    imageSum.value = 9 - calculateNum.imageLength
...
const confirmText = (val, obj) => {
    console.log('val', val, textSum.value)
    // 如果附件基数（textSum/otherSum）已达到，则停止弹窗
    if (val == 'TEXT' && textSum.value === 0) {
        console.log('111111')
        return
    }
    if (val != 'TEXT' && val != 'IMAGE' && otherSum.value === 0) {
        console.log('222222')
        return
    }
    if ((imageSum.value === 0 || calculateNum.imageLength === 0 && hasMedia.value) && val !== 'TEXT') {
        console.log('333333')
        return
    }
    console.log('obj', obj)
...
// 判断按钮置灰
const getDisabled = () => {
    if (calculateNum.textLength > 0 && (calculateNum.fileLength > 0 || calculateNum.videoLength > 0 || calculateNum.webLinkLength > 0)) {
        return true
