...
export default {
    ...
    methods: {
        ...
        async handleLogin() {
            console.log(this.$refs.loginForm.validate(valid => console.log(valid)))
            ...
            this.$store.dispatch('user/login', this.loginForm).then(() => {
                ...
                if (this.loginForm.remember) {
                    localStorage.setItem('login_account', this.loginForm.username)
                } else {
                    localStorage.removeItem('login_account')
                }
                this.$store.dispatch('user/getMenus').then(data => {
                    if (data.length === 0) {
                        ElMessage.error('暂无页面权限，请管理员配置权限')
                    } else {
                        ElMessage.success('登录成功')
                        this.$store.dispatch('user/getSubjectName').then(() => {
                            this.$router.push({ path: this.$route.query.redirect || '/' })
                        })
                    }

                })
                // this.$store.dispatch('user/getSubjectName')
