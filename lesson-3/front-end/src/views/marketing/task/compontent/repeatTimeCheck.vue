            ...
            <span class="check-time-content mr8">
                ...
                >
                    <div class="show-time-box">
                        <div v-for="(item,index) in props.timeData" :key="index" class="time-select-box mt6" @click="checkTime(item, index)">
                            {{ item.showTime }}
                        </div>
                    </div>
                    <!-- <el-table 
...
function checkoutTime(num) {
    if (activeIndex.value === 0 && num === -1 || activeIndex.value === (props.timeData.length - 1) && num === 1) {
