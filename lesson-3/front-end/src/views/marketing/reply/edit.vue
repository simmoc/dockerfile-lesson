        ...
        <div>
            ...
            >
                <div class="head-content-item">
                    创建成员：{{
                        (eventDetail && eventDetail.creator_name) ||
...
onMounted(() => {
    if (route.query.group_id && route.query.sale_scrip_id) getKnowledgeDetails()
    if (route.query.group_id) {
        addknowInfo.value.group_id = Number(route.query.group_id)
    }
    if (route.query.eventDetail) {
        let obj = JSON.parse(route.query.eventDetail)
        eventDetail.value.create_time = obj.create_time
        eventDetail.value.member_name = obj.member_name
        eventDetail.value.update_time = obj.update_time
        eventDetail.value.sale_scrip_id = obj.sale_scrip_id
        addknowInfo.value.sale_script_id = obj.sale_scrip_id
        getKnowledgeDetails()
    }
})
const cancelData = () => {
...
const getKnowledgeDetails = async() => {
    ...
    let { data } = await Http
        .getKnowledgeDetails({
            sale_script_id: eventDetail.value.sale_scrip_id
        })
...
const sumitKnowdeage = async() => {
    ...
    addknowInfo.value.content_arr = content_arr.value
    // 2.数据校验
    // 3.发送请求
    searchKeysVal.value.forEach(item => {
        addknowInfo.value.keyword_arr.push({ name: item })
    })
    let routeID = JSON.parse(route.query.eventDetail)
    console.error('route.query', routeID)
    let res = ''
    if (route.query.group_id && routeID.sale_scrip_id) {
        res = await Http.updateKnowledge(addknowInfo.value)
        console.error('111111', res)
    }
    if (res.code === 200) {
        ElMessage.success({
            message: '提交成功'
    ...
    // Http.addKnowledgeSpeech(addknowInfo.value)
    // ElMessage.success({
    //     message: '提交成功'
    // })
...
const searchKeysKnowledgeList = () => {
    ...
    if (
        searchKeysVal.value.length &&
        searchKeysVal.value[searchKeysVal.value.length - 1].length > 5
    ) {
        searchKeysVal.value.pop()
        ElMessage.error({
            message: '关键词长度不能超过5个字符!'
        })
    }
    console.log('searchKeysVal.value', searchKeysVal.value)
}
const getKeys = val => {
    if (val.length > 5) {
    ...
    if (val) {
        let idx = keysList.value.findIndex(val => { val.name === val })
        if (idx === -1) {
