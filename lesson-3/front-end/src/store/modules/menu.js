...
function hasMenu(menus, menu) {
    ...
    menus.forEach((item, index) => {
        // 客户模块比对path拿到后端的title与subject_id
        if (item.page_route && item.page_route.indexOf('customer') !== -1 && item.page_route.indexOf('iframe') === -1) {
            if (item.page_route === menu.meta.activeMenu) {
                menu.meta.title = item.title
                menu.meta.subject_id = item.subject_id
                i = index
            }
        } else {
            if (item.title === menu.meta.title) {
                i = index
            }
        }
    })
