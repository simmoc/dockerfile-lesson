...
// import mvpInlineIframe from '@/components/Layout/inline-iframe.vue'
// import { iframeRoutes } from '../router/iframes'
...
// const inlineInclude =  reactive([])
// const include = reactive([])
// const handleTabsDataChange = (tabs) => {
//     inlineInclude = tabs
//     .filter((tab) => iframeNames.indexOf(tab.name) !== -1)
//     .map((tab) => tab.name)
//     include = tabs.map((tab) => tab.name)
// }
...
function switchMenu(index) {
    ...
    if (store.state.settings.switchSidebarAndPageJump) {
        router.push(store.getters['menu/sidebarRoutes'][0].path + '/' + store.getters['menu/sidebarRoutes'][0].children[0].path)
    }
}
</script>
