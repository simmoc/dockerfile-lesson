    ...
    <div v-if="item.meta.sidebar !== false" class="sidebar-item">
        ...
        >
            ...
            >
                <el-menu-item :title="item.meta.title" :index="resolvePath(item.path)">
                    <!--                    <svg-icon v-if="iconName(isActive || isExactActive, item.meta.icon)"-->
                    <!--                              :name="iconName(isActive || isExactActive, item.meta.icon)" class="icon"-->
                    <!--                    />-->
                    <!--                    <svg-icon v-if="item.meta.icon"-->
                    <!--                              :name="item.meta.icon" class="icon"-->
                    <!--                    />-->
                    <img v-if="item.meta.icon" :src="getImageUrl(isActive?item.meta.icon+'_check':item.meta.icon)" :name="item.meta.icon" alt="" class="icon">
                    <span class="title">{{ item.meta.title }}</span>
                </el-menu-item>
            </a>
        </router-link>
        <el-sub-menu v-else :title="item.meta.title" :index="resolvePath(item.path)">
            <template #title>
                ...
                <img v-if="item.meta.icon" :src="getImageUrl(item.meta.icon)" :name="item.meta.icon" alt="" class="icon unactive">
                <!--                <svg-icon v-if="item.meta.activeIcon || item.meta.icon" :name="item.meta.activeIcon || item.meta.icon"-->
                <!--                          class="icon active"-->
                <!--                />-->
                <img v-if="item.meta.icon" :src="getImageUrl(item.meta.icon+'_check')" :name="item.meta.icon" alt="" class="icon active">
                <span class="title">{{ item.meta.title }}</span>
            </template>
            <SidebarItem v-for="route in item.children" :key="route.path" :item="route"
...
function getImageUrl(name) {
    // console.error('11111', import.meta.url)
...
// }
// 不行，路径变成了一个[object promise]
// const fileUrl = computed(async() => {
//     let img = await import('./assets/images/icon/' + props.item.meta.icon + '.png')
...
<style lang="scss">
    ...
    .el-sub-menu .el-menu--inline .el-menu-item.is-active {
        ...
        border-right: 2px solid $g-sub-sidebar-menu-active-color !important;
        font-weight: bold;
        .icon,
    ...
    .el-menu[role='menubar'] > .sidebar-item > .el-sub-menu.is-active > .el-sub-menu__title:first-child {
        color: $g-sub-sidebar-menu-active-color !important;
        font-weight: bold;
    }
</style>
